...
class BookViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        serializer.save(owner=self.request.user, total_pages=total_pages)
    ...
    def peer_books(self, request, pk=None):
        ...
        my_aladin_ids = Book.objects.filter(owner=request.user).values_list(
            "aladin_item_id", flat=True
        )

        aggregated = (
            Book.objects.filter(owner_id__in=peer_owner_ids)
            .exclude(aladin_item_id=book.aladin_item_id)
            .exclude(aladin_item_id__in=my_aladin_ids)
            .values("aladin_item_id")
            .annotate(reader_count=Count("owner", distinct=True))
...
class BookQuoteViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        qs = BookQuote.objects.filter(book__owner=self.request.user).select_related("book")
    ...
    def create(self, request, *args, **kwargs):
        ...
        book = Book.objects.filter(id=book_id, owner=request.user).first()
        if not book:
